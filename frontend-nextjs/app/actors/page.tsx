...
import { actorAPI, favoriteAPI } from '@/lib/api';
...
export default function ActorsPage() {
  ...
  }, [paginationEnabled, loadingMore, loading, actors.length]);
  ...
  const loadCurrentPage = async () => {
    ...
    try {
      ...
      const response = await actorAPI.getActors(params);
  ...
  const loadActors = async (reset: boolean = false) => {
    if (reset) {
      setLoading(true);
      setActors([]);
      setPage(1);
    ...
    try {
      ...
      if (reset) {
        ...
        setActors(prev => [...prev, ...response.data.items]);
        setPage(currentPage);
      }
      ...
      <div className="bg-white border-b flex-shrink-0">
        <div className="container mx-auto px-4 py-3">
          ...
          <div className="flex items-center justify-between mb-3">
            <h1 className="text-2xl sm:text-3xl font-bold">Actors</h1>
      ...
      <div className="flex-1 overflow-y-auto bg-gray-50">
        <div className="container mx-auto px-4 py-4">
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-500">No actors found</div>
          ) : (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-4">
                {actors.map((actor) => (
                  <ActorCard key={actor.id} actor={actor} onClick={() => handleActorClick(actor.id)} />
                ))}
              </div>
              ...
              {!paginationEnabled && (
                <div ref={observerTarget} className="py-4 text-center">
                  {loadingMore && <Loading />}
                  {!hasMore.current && <div className="text-gray-500 text-sm">No more actors</div>}
                </div>
              )}
            </>
          )}
...
const ActorCard = ({ actor, onClick }: { actor: Actor; onClick: () => void }) => {
  ...
  const checkFavoriteStatus = async () => {
    try {
      const response = await favoriteAPI.checkFavorite('actor', actor.id);
  ...
  const toggleFavorite = async (e: React.MouseEvent) => {
    ...
    try {
      if (isFavorite) {
        await favoriteAPI.removeFavorite('actor', actor.id);
        setIsFavorite(false);
      } else {
        await favoriteAPI.addFavorite('actor', actor.id);
        setIsFavorite(true);
      }
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      {/* Info Section */}
      <div className="p-2.5">
        <h3 className="font-semibold text-sm line-clamp-1 mb-1.5">{actor.name}</h3>
        
        <div className="flex items-center justify-between text-xs">
          {actor.nationality && (
            <span className="text-gray-600 truncate">{actor.nationality}</span>
          )}
          {actor.birth_date && (
