...
import { Star, ChevronLeft, ChevronRight, Heart, Filter, X } from 'lucide-react'
import { movieAPI, genreAPI, favoriteAPI } from '../services/api'
...
// Move FilterInputs OUTSIDE the component to prevent recreation
const FilterInputs = ({ 
  filters, 
  genres, 
  onFilterChange 
...
export default function MoviesPage() {
  ...
  }, [paginationEnabled, loadingMore, loading, movies.length])

  const loadGenres = async () => {
    try {
      const response = await genreAPI.getGenres({ page_size: 100 })
            ...
            <>
              ...
              {!paginationEnabled && (
                <div ref={observerTarget} className="py-4 text-center">
                  {loadingMore && <Loading />}
                  {!hasMore.current && <div className="text-gray-500 text-sm">No more movies</div>}
                </div>
              )}
            </>
          )}
...
const MovieCard = ({ movie, onClick }: { movie: Movie; onClick: () => void }) => {
  ...
  const checkFavoriteStatus = async () => {
    try {
      const response = await favoriteAPI.checkFavorite('movie', movie.id)
  ...
  const toggleFavorite = async (e: React.MouseEvent) => {
    ...
    try {
      if (isFavorite) {
        await favoriteAPI.removeFavorite('movie', movie.id)
        setIsFavorite(false)
      } else {
        await favoriteAPI.addFavorite('movie', movie.id)
        setIsFavorite(true)
      }
    } catch (error) {
