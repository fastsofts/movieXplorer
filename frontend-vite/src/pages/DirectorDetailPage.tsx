...
export default function DirectorDetailPage() {
  ...
  useEffect(() => {
    if (id) {
      loadDirectorDetails(parseInt(id))
    }
  }, [id, actorIdFromQuery])
  ...
  const loadDirectorDetails = async (directorId: number) => {
    setLoading(true)
    try {
      const [directorRes, actorsRes, moviesRes] = await Promise.all([
        directorAPI.getDirector(directorId),
        directorAPI.getDirectorActors(directorId),
        directorAPI.getDirectorMovies(directorId, actorIdFromQuery ? parseInt(actorIdFromQuery) : undefined)
      ])
  ...
  if (!director) return <div className="h-full flex items-center justify-center">Director not found</div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {actors.map((actor: any) => (
                ...
                >
                  ...
                  <p className="text-xs text-gray-500 text-center">{actor.movie_count} movies</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {movies.length > 0 && (
          <div className="bg-white rounded-lg shadow-lg p-6">
            <h2 className="text-2xl font-bold mb-4">
              {actorIdFromQuery ? 'Movies with Selected Actor' : 'All Movies'}
            </h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
